...
<div class="space-y-6 p-6" x-data="{
                    ...
                    <tr class="hover:bg-slate-50 transition">
                        ...
                        <td class="px-4 py-3.5 text-right pr-4">
                            <div class="inline-flex items-center justify-end gap-1.5">
                                ...
                                <button @click="initDelete({ id: {{ $acc->id }}, account_name: '{{ addslashes($acc->account_name) }}' })" class="p-2 rounded-lg bg-red-600/10 hover:bg-red-600/20 text-red-600 hover:text-red-700 transition inline-flex items-center justify-center shadow-sm" title="Delete Account">
    ...
    <div x-show="openViewModal" x-cloak x-transition.opacity style="display: none;" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-xs p-4">
        <div class="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl space-y-5" @click.outside="openViewModal = false">
            <div class="flex items-center justify-between border-b border-slate-100 pb-3">
                <h3 class="font-bold text-slate-900 text-base">Account Head Details</h3>
            ...
            <div class="space-y-3 text-xs">
                <div class="flex justify-between border-b border-slate-100 pb-2">
                    ...
                    <span class="font-bold font-mono text-slate-900" x-text="viewAccount.account_code"></span>
                </div>
                <div class="flex justify-between border-b border-slate-100 pb-2">
    ...
    <div x-show="openAddModal" x-cloak x-transition.opacity style="display: none;" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-xs p-4">
        <div class="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl space-y-5" @click.outside="openAddModal = false">
            <div class="flex items-center justify-between border-b border-slate-100 pb-3">
                <h3 class="font-bold text-slate-900 text-base">Add New Account Head</h3>
            ...
            <form action="{{ route('chart-of-accounts.store') }}" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="account_code" required placeholder="e.g., 1005" class="w-full px-3 py-2 text-xs border border-slate-250 rounded-lg focus:ring-2 focus:ring-[#a38c29]">
                </div>
                <div>
                    <label class="block text-xs font-bold text-slate-700 mb-1">Account Name *</label>
                    <input type="text" name="account_name" required placeholder="e.g., Office Reserve Fund" class="w-full px-3 py-2 text-xs border border-slate-250 rounded-lg focus:ring-2 focus:ring-[#a38c29]">
                </div>
                <div>
                    <label class="block text-xs font-bold text-slate-700 mb-1">Account Type *</label>
                    <select name="account_type" required class="w-full px-3 py-2 text-xs border border-slate-250 rounded-lg focus:ring-2 focus:ring-[#a38c29]">
                ...
                <div class="flex items-center gap-2 pt-2">
                    <input type="checkbox" name="is_active" id="add_is_active" value="1" checked class="rounded border-slate-300 text-[#a38c29] focus:ring-[#a38c29]">
                    <label for="add_is_active" class="text-xs font-semibold text-slate-700">Active Account</label>
                </div>
                <div class="flex justify-end gap-2 pt-4">
                    ...
                    <button type="submit" class="px-4 py-2 bg-[#a38c29] hover:bg-[#8a7522] text-white text-xs font-bold rounded-lg shadow-sm">Save Account</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div x-show="openEditModal" x-cloak x-transition.opacity style="display: none;" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-xs p-4">
        <div class="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl space-y-5" @click.outside="openEditModal = false">
            <div class="flex items-center justify-between border-b border-slate-100 pb-3">
                <h3 class="font-bold text-slate-900 text-base">Edit Account Head</h3>
                <button @click="openEditModal = false" class="text-slate-400 hover:text-slate-600">✕</button>
            </div>
            <form :action="'/chart-of-accounts/' + editAccount.id" method="POST" class="space-y-4">
                @csrf
                @method('PUT')
                <div>
                    <label class="block text-xs font-bold text-slate-700 mb-1">Account Code *</label>
                    <input type="text" name="account_code" x-model="editAccount.account_code" required class="w-full px-3 py-2 text-xs border border-slate-250 rounded-lg focus:ring-2 focus:ring-[#a38c29]">
                </div>
                <div>
                    <label class="block text-xs font-bold text-slate-700 mb-1">Account Name *</label>
                    <input type="text" name="account_name" x-model="editAccount.account_name" required class="w-full px-3 py-2 text-xs border border-slate-250 rounded-lg focus:ring-2 focus:ring-[#a38c29]">
                </div>
                <div>
                    <label class="block text-xs font-bold text-slate-700 mb-1">Account Type *</label>
                    <select name="account_type" x-model="editAccount.account_type" required class="w-full px-3 py-2 text-xs border border-slate-250 rounded-lg focus:ring-2 focus:ring-[#a38c29]">
                        <option value="ASSET">ASSET</option>
                        <option value="LIABILITY">LIABILITY</option>
                        <option value="REVENUE">REVENUE</option>
                        <option value="EXPENSE">EXPENSE</option>
                ...
                <div class="flex items-center gap-2 pt-2">
                    <input type="checkbox" name="is_active" id="edit_is_active" value="1" :checked="editAccount.is_active" class="rounded border-slate-300 text-[#a38c29] focus:ring-[#a38c29]">
                    <label for="edit_is_active" class="text-xs font-semibold text-slate-700">Active Account</label>
                </div>
                <div class="flex justify-end gap-2 pt-4">
                    ...
                    <button type="submit" class="px-4 py-2 bg-[#a38c29] hover:bg-[#8a7522] text-white text-xs font-bold rounded-lg shadow-sm">Update Account</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div x-show="openDeleteModal" x-cloak x-transition.opacity style="display: none;" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-xs p-4">
        <div class="bg-white rounded-2xl max-w-sm w-full p-6 shadow-2xl space-y-4" @click.outside="openDeleteModal = false">
            <div class="text-center">
                ...
                <h3 class="font-bold text-slate-900 text-base">Delete Account Head</h3>
                <p class="text-xs text-slate-500 mt-1">Are you sure you want to delete <span class="font-bold text-slate-800" x-text="deleteAccount.account_name"></span>?</p>
            </div>
            <form :action="'/chart-of-accounts/' + deleteAccount.id" method="POST" class="flex justify-center gap-2 pt-2">
                @csrf
                @method('DELETE')
