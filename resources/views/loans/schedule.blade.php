        ...
        <div class="relative w-full max-w-md bg-white rounded-2xl shadow-xl overflow-hidden animate-fade-in-up">
            ...
            <form @submit.prevent="submitPayForm">
                <div class="p-6 space-y-4">
                    ...
                    <div>
                        <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1.5">Payment From (Bank / Cash) *</label>
                        <select x-model="payForm.bank_account_id" required class="w-full px-3 py-2 bg-slate-50 border border-slate-200 focus:bg-white focus:ring-2 focus:ring-[#a38c29]/20 rounded-xl text-xs text-slate-800 focus:outline-none transition-all cursor-pointer">
                            <option value="">Select account to pay from...</option>
                            @foreach($assetAccounts as $acc)
                                <option value="{{ $acc->id }}">{{ $acc->name }} ({{ strtoupper($acc->code) }})</option>
                            @endforeach
                        </select>
                    </div>
                </div>
