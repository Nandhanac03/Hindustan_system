                        ...
                        <tr class="hover:bg-slate-50/50 transition-colors">
                            ...
                            <td class="px-4 py-3 border text-right">
                                <div class="inline-flex items-center justify-end gap-1.5">
                                    ...
                                    <button @click="openEditModal({{ $type->id }}, {{ $type->project_id ?? 'null' }}, '{{ addslashes($type->name) }}', '{{ addslashes($type->category) }}', {{ $type->is_active ? 'true' : 'false' }})" title="Edit Unit Type" class="p-2 rounded-lg bg-[#a38c29]/10 hover:bg-[#a38c29]/20 text-[#a38c29] hover:text-[#8a7522] transition inline-flex items-center justify-center shadow-sm">
                                    ...
                                    @if($type->units_count === 0)
                                        <form action="{{ route('unit-types.destroy', $type->id) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this unit type?');">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" title="Delete Unit Type" class="p-2 rounded-lg bg-[#a38c29]/10 hover:bg-[#a38c29]/20 text-[#a38c29] hover:text-[#8a7522] transition inline-flex items-center justify-center shadow-sm">
