...
<div x-data="raBillManagement()" class="p-6 space-y-6 bg-slate-50 min-h-screen">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse min-w-[1350px]">
                <thead class="bg-[#a38c29] text-white border-b border-[#8a7522] text-[10px] font-black uppercase tracking-wider sticky top-0 z-10 shadow-2xs">
                    <tr>
                        <th class="px-2.5 py-3.5 text-center w-[95px] min-w-[95px]">RA BILL NO</th>
                        <th class="px-3.5 py-3.5 text-left w-[210px] min-w-[190px]">CONTRACTOR / PROJECT / UNIT</th>
                        <th class="px-2.5 py-3.5 text-center w-[105px] min-w-[100px]">SUBMIT DATE</th>
                        <th class="px-3.5 py-3.5 text-right w-[130px] min-w-[120px]">RA BILL AMOUNT</th>
                        <th class="px-2.5 py-3.5 text-center w-[130px] min-w-[120px]">VERIFIED DATE</th>
                        <th class="px-3.5 py-3.5 text-right w-[125px] min-w-[115px]">CORRECTION</th>
                        <th class="px-3.5 py-3.5 text-right bg-[#8a7522]/40 w-[140px] min-w-[130px]">AFTER CORRECTION</th>
                        <th class="px-2.5 py-3.5 text-center w-[100px] min-w-[95px]">DUE DATE</th>
                        <th class="px-2.5 py-3.5 text-center w-[105px] min-w-[100px]">PAID DATE</th>
                        <th class="px-3.5 py-3.5 text-right text-emerald-200 w-[130px] min-w-[120px]">PAID AMOUNT</th>
                        <th class="px-3.5 py-3.5 text-right text-rose-200 w-[135px] min-w-[125px]">BALANCE AMOUNT</th>
                        <th class="px-2.5 py-3.5 text-center w-[165px] min-w-[155px]">STATUS</th>
                        <th class="px-3 py-3.5 text-center w-[140px] min-w-[130px]">ACTIONS</th>
                    </tr>
                </thead>
                    ...
                    @forelse($raBills as $bill)
                        ...
                        @if($totalInstallments === 0)
                            ...
                            <tr class="hover:bg-amber-50/20 transition-colors border-b-2 border-slate-200">
                                <td class="px-2.5 py-3.5 text-center align-middle border-r border-slate-200/50 bg-slate-50/50">
                                    <span class="inline-block px-2 py-0.5 bg-slate-200/80 text-slate-900 rounded font-mono font-extrabold text-[11px] whitespace-nowrap shadow-2xs">{{ $bill->ra_bill_number }}</span>
                                </td>
                                ...
                                <td class="px-3.5 py-3.5 align-middle">
                                    ...
                                    @if($bill->unit_name || $bill->unit)
                                        <div class="mt-1">
                                            <span class="inline-flex items-center gap-1 px-1.5 py-0.5 bg-amber-100/90 text-amber-950 border border-amber-300/70 rounded text-[9px] font-black uppercase tracking-wider whitespace-nowrap shadow-2xs">
                                ...
                                <td class="px-2.5 py-3.5 text-center text-slate-600 font-mono align-middle">
                                    {{ $bill->submit_date ? $bill->submit_date->format('d/m/Y') : '—' }}
                                </td>

                                <td class="px-3.5 py-3.5 text-right font-mono font-bold text-slate-900 align-middle">
                                    ₹{{ number_format((float) $bill->gross_amount, 2) }}
                                </td>

                                <td class="px-2.5 py-3.5 text-center font-mono align-middle">
                                    @if($bill->verified_date)
                                        <span class="text-emerald-700 font-bold block whitespace-nowrap">{{ $bill->verified_date->format('d/m/Y') }}</span>
                                        <div class="text-[10px] text-slate-500 font-semibold mt-0.5 truncate max-w-[115px] mx-auto" title="Verified By: {{ $bill->engineer_name }}">By: {{ $bill->engineer_name ?: 'Engineer' }}</div>
                                    @else
                                        <span class="text-amber-600 italic font-medium">Unverified</span>
                                    @endif
                                </td>

                                <td class="px-3.5 py-3.5 text-right font-mono text-amber-700 font-bold align-middle">
                                    {{ (float)$bill->correction_amount > 0 ? '-₹' . number_format((float)$bill->correction_amount, 2) : '₹0.00' }}
                                </td>

                                <td class="px-3.5 py-3.5 text-right font-mono font-black text-blue-900 bg-blue-50/30 align-middle">
                                    ₹{{ number_format((float) $bill->net_approved_amount, 2) }}
                                </td>

                                <td class="px-2.5 py-3.5 text-center font-mono text-slate-600 align-middle">
                                    {{ $bill->due_date ? $bill->due_date->format('d/m/Y') : '—' }}
                                </td>

                                <td class="px-2.5 py-3.5 text-center font-mono text-slate-400 align-middle">—</td>
                                <td class="px-3.5 py-3.5 text-right font-mono font-bold text-slate-400 align-middle">₹0.00</td>
                                <td class="px-3.5 py-3.5 text-right font-mono font-black text-rose-700 align-middle">
                                    ₹{{ number_format((float) $bill->balance_amount, 2) }}
                                </td>

                                <td class="px-2.5 py-3.5 text-center whitespace-nowrap align-middle">
                                    @if($bill->verified_date)
                                        <span class="px-2.5 py-1 rounded-full text-[10px] font-black bg-amber-50 text-amber-900 border border-amber-300 inline-flex items-center gap-1 shadow-2xs uppercase tracking-wider">
                                            <span class="w-1.5 h-1.5 rounded-full bg-amber-600"></span>
                                            <span>PENDING</span>
                                        </span>
                                    @else
                                        <span class="px-2.5 py-1 rounded-full text-[10px] font-bold bg-slate-100 text-slate-700 border border-slate-200 inline-flex items-center gap-1 shadow-2xs uppercase tracking-wider">
                                    ...
                                    <div class="inline-flex items-center justify-center gap-1.5">
                                        @if($bill->verified_date)
                                            <button type="button" @click="openVerifyModal({{ json_encode($bill) }})"
                                                    class="px-2.5 py-1 bg-gradient-to-r from-[#a38c29] via-[#947e24] to-[#8a7522] hover:from-[#8a7522] hover:to-[#73611c] text-white rounded-full text-[11px] font-bold transition inline-flex items-center gap-1 shadow-2xs cursor-pointer border border-[#a38c29]/40"
                                        ...
                                        @else
                                            <button type="button" @click="openVerifyModal({{ json_encode($bill) }})"
                                                    class="px-2.5 py-1 bg-[#a38c29] hover:bg-[#8a7522] text-white rounded-full text-[11px] font-bold transition inline-flex items-center gap-1 shadow-2xs cursor-pointer"
                                        ...
                                        @if($bill->verified_date && (float)$bill->balance_amount > 0)
                                            <button type="button" @click="openDisburseModal({{ json_encode($bill) }})"
                                                    class="px-2.5 py-1 bg-emerald-600 hover:bg-emerald-700 text-white rounded-full text-[11px] font-bold transition inline-flex items-center gap-1 shadow-2xs cursor-pointer"
                        ...
                        @else
                            ...
                            @foreach($payments as $pIdx => $payment)
                                ...
                                <tr class="hover:bg-amber-50/20 transition-colors {{ $isLastPayment ? 'border-b-2 border-slate-200' : 'border-b border-slate-100' }} {{ $pIdx > 0 ? 'bg-slate-50/30' : '' }}">
                                    @if($pIdx === 0)
                                        <td class="px-2.5 py-3.5 text-center align-middle border-r border-slate-200/50 bg-slate-50/50" rowspan="{{ $rowspan }}">
                                            <span class="inline-block px-2 py-0.5 bg-slate-200/80 text-slate-900 rounded font-mono font-extrabold text-[11px] whitespace-nowrap shadow-2xs">{{ $bill->ra_bill_number }}</span>
                                        </td>

                                        <td class="px-3.5 py-3.5 align-middle" rowspan="{{ $rowspan }}">
                                            <div class="font-black text-slate-900 text-xs leading-tight">{{ $bill->contractor_name ?: ($bill->contractor->name ?? 'General Contractor') }}</div>
                                            <div class="text-[10px] text-slate-500 font-semibold mt-0.5 leading-tight">{{ $bill->project->name ?? 'Site Project' }}</div>
                                            @if($bill->unit_name || $bill->unit)
                                                <div class="mt-1">
                                        ...
                                        <td class="px-2.5 py-3.5 text-center text-slate-600 font-mono align-middle" rowspan="{{ $rowspan }}">
                                            {{ $bill->submit_date ? $bill->submit_date->format('d/m/Y') : '—' }}
                                        </td>

                                        <td class="px-3.5 py-3.5 text-right font-mono font-bold text-slate-900 align-middle" rowspan="{{ $rowspan }}">
                                            ₹{{ number_format((float) $bill->gross_amount, 2) }}
                                        </td>

                                        <td class="px-2.5 py-3.5 text-center font-mono align-middle" rowspan="{{ $rowspan }}">
                                            @if($bill->verified_date)
                                                <span class="text-emerald-700 font-bold block whitespace-nowrap">{{ $bill->verified_date->format('d/m/Y') }}</span>
                                                <div class="text-[10px] text-slate-500 font-semibold mt-0.5 truncate max-w-[115px] mx-auto" title="Verified By: {{ $bill->engineer_name }}">By: {{ $bill->engineer_name ?: 'Engineer' }}</div>
                                            @else
                                        ...
                                        <td class="px-3.5 py-3.5 text-right font-mono text-amber-700 font-bold align-middle" rowspan="{{ $rowspan }}">
                                            {{ (float)$bill->correction_amount > 0 ? '-₹' . number_format((float)$bill->correction_amount, 2) : '₹0.00' }}
                                        </td>

                                        <td class="px-3.5 py-3.5 text-right font-mono font-black text-blue-900 bg-blue-50/30 align-middle" rowspan="{{ $rowspan }}">
                                            ₹{{ number_format((float) $bill->net_approved_amount, 2) }}
                                        </td>

                                        <td class="px-2.5 py-3.5 text-center font-mono text-slate-600 align-middle" rowspan="{{ $rowspan }}">
                                            {{ $bill->due_date ? $bill->due_date->format('d/m/Y') : '—' }}
                                        </td>
                                    @endif

                                    <!-- Installment Row Specific Columns -->
                                    <td class="px-2.5 py-3 text-center font-mono text-slate-800 font-bold align-middle">
                                        {{ $payment->payment_date ? $payment->payment_date->format('d/m/Y') : '—' }}
                                    </td>

                                    <td class="px-3.5 py-3 text-right font-mono font-bold text-emerald-700 align-middle">
                                        ₹{{ number_format($paidInst, 2) }}
                                    </td>

                                    <td class="px-3.5 py-3 text-right font-mono font-black align-middle {{ $isCleared ? 'text-slate-500' : 'text-rose-700' }}">
                                        ₹{{ number_format($runningBal, 2) }}
                                    </td>

                                    <!-- Excel Matched Status Column -->
                                    <td class="px-2.5 py-3 text-center whitespace-nowrap align-middle">
                                        @if($isCleared)
                                            <span class="px-2.5 py-1 rounded-full text-[10px] font-black bg-[#ECFDF3] text-[#065F46] border border-[#A7F3D0] inline-flex items-center gap-1 shadow-2xs uppercase tracking-wider whitespace-nowrap">
                                                <svg class="w-3 h-3 text-[#087443] shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"/></svg>
                                                <span>CLEARED (RA #{{ $bill->ra_bill_number }})</span>
                                            </span>
                                        @else
                                            <span class="px-2.5 py-1 rounded-full text-[10px] font-black bg-amber-50 text-amber-900 border border-amber-300 inline-flex items-center gap-1 shadow-2xs uppercase tracking-wider whitespace-nowrap">
                                    ...
                                    @if($pIdx === 0)
                                        <td class="px-3 py-3.5 text-center whitespace-nowrap align-middle" rowspan="{{ $rowspan }}">
                                            <div class="inline-flex items-center justify-center gap-1.5">
                                                @if($bill->verified_date)
                                                    <button type="button" @click="openVerifyModal({{ json_encode($bill) }})"
                                                            class="px-2.5 py-1 bg-gradient-to-r from-[#a38c29] via-[#947e24] to-[#8a7522] hover:from-[#8a7522] hover:to-[#73611c] text-white rounded-full text-[11px] font-bold transition inline-flex items-center gap-1 shadow-2xs cursor-pointer border border-[#a38c29]/40"
        ...
        <div class="bg-white rounded-2xl max-w-xl w-full shadow-2xl overflow-hidden border border-slate-200" @click.away="addModalOpen = false">
            ...
            <form action="{{ route('expenses.ra-bills.store') }}" method="POST" class="p-5 space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-xs font-bold uppercase tracking-wider mb-1.5 {{ $errors->has('ra_bill_number') ? 'text-rose-600' : 'text-slate-700' }}">RA BILL NO <span class="text-rose-500 font-bold">*</span></label>
                    ...
                    <div>
                        <label class="block text-xs font-bold uppercase tracking-wider mb-1.5 {{ $errors->has('submit_date') ? 'text-rose-600' : 'text-slate-700' }}">CONTRACTOR SUBMIT DATE <span class="text-rose-500 font-bold">*</span></label>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-xs font-bold uppercase tracking-wider mb-1.5 {{ $errors->has('contractor_id') ? 'text-rose-600' : 'text-slate-700' }}">CONTRACTOR NAME <span class="text-rose-500 font-bold">*</span></label>
                    ...
                    <div>
                        <label class="block text-xs font-bold uppercase tracking-wider mb-1.5 {{ $errors->has('project_id') ? 'text-rose-600' : 'text-slate-700' }}">SITE PROJECT <span class="text-rose-500 font-bold">*</span></label>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-xs font-bold uppercase tracking-wider mb-1.5 {{ $errors->has('unit_id') ? 'text-rose-600' : 'text-slate-700' }}">UNIT <span class="text-rose-500 font-bold">*</span></label>
                    ...
                    <div>
                        <label class="block text-xs font-bold uppercase tracking-wider mb-1.5 {{ $errors->has('gross_amount') ? 'text-rose-600' : 'text-slate-700' }}">RA BILL GROSS AMOUNT (₹) <span class="text-rose-500 font-bold">*</span></label>
                ...
                <div class="p-4 bg-white border border-amber-200/80 rounded-2xl shadow-2xs grid grid-cols-3 gap-4 items-center">
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 rounded-xl bg-amber-50 text-[#a38c29] border border-amber-200/60 flex items-center justify-center shrink-0">
                            {{-- FIX #1: repaired malformed SVG path data.
                                 Original had "a1 1 0 011 v5" (missing digit) which is invalid path
                                 syntax and caused this icon to fail to render. --}}
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/></svg>
                        </div>
                        <div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-xs font-extrabold text-slate-700 uppercase tracking-wider mb-1.5 flex items-center gap-1.5">
                            ...
                            <span>VERIFIED DATE <span class="text-rose-500 font-bold">*</span></span>
                        </label>
                    ...
                    <div>
                        <label class="block text-xs font-extrabold text-slate-700 uppercase tracking-wider mb-1.5 flex items-center gap-1.5">
                            ...
                            <span>SITE ENGINEER (FROM MASTER) <span class="text-rose-500 font-bold">*</span></span>
                        </label>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-xs font-extrabold text-amber-900 uppercase tracking-wider mb-1.5 flex items-center gap-1.5">
                            ...
                            <span>CORRECTION OF BILL (DEDUCTION ₹) <span class="text-rose-500 font-bold">*</span></span>
                        </label>
    ...
    <div x-show="disburseModalOpen" x-cloak class="fixed inset-0 z-50 overflow-y-auto bg-slate-900/65 backdrop-blur-md flex items-center justify-center p-4">
        <div class="bg-white rounded-3xl max-w-2xl w-full shadow-2xl overflow-hidden border border-slate-100 transform transition-all" @click.away="disburseModalOpen = false">

            <!-- Hero Gradient Header -->
            <div class="relative bg-gradient-to-r from-emerald-800 via-emerald-700 to-emerald-900 p-6 text-white overflow-hidden">
                ...
                <div class="flex items-center justify-between relative z-10">
                    <div class="flex items-center gap-3.5">
                        <div class="w-12 h-12 rounded-2xl bg-white/20 backdrop-blur-md border border-white/30 flex items-center justify-center shadow-inner">
                            <div class="w-8 h-8 rounded-xl bg-white text-emerald-700 flex items-center justify-center shadow-md">
                                <svg class="w-5 h-5 text-emerald-700" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z"/></svg>
                            </div>
                        </div>
                        <div>
                            <h3 class="font-black text-base uppercase tracking-wider text-white">Disburse Staggered Payment</h3>
                            <p class="text-xs text-emerald-100 font-medium mt-0.5">Generate official Payment Voucher and release corporate treasury funds.</p>
                        </div>
                    </div>
                    <button type="button" @click="disburseModalOpen = false"
                            class="w-9 h-9 rounded-full bg-white/20 hover:bg-white/30 text-white flex items-center justify-center transition cursor-pointer font-bold">✕</button>
                </div>
            </div>

            <form :action="selectedBill ? `/expenses/ra-bills/${selectedBill.id}/disburse` : '#'" method="POST" class="p-6 space-y-5">
                @csrf

                <!-- Bill Summary Card -->
                <div class="bg-gradient-to-br from-emerald-50 via-slate-50 to-emerald-50/40 p-4 rounded-2xl border border-emerald-200/70 shadow-2xs">
                    <div class="grid grid-cols-3 gap-3 text-center">
                        <div class="border-r border-emerald-200/60 pr-2">
                            <span class="block text-[10px] font-extrabold text-emerald-800 uppercase tracking-widest">RA BILL NO.</span>
                            <span class="text-xs font-mono font-black text-slate-900 mt-0.5 block" x-text="selectedBill ? selectedBill.ra_bill_number : ''"></span>
                        </div>
                        <div class="border-r border-emerald-200/60 pr-2">
                            <span class="block text-[10px] font-extrabold text-emerald-800 uppercase tracking-widest">NET APPROVED</span>
                        ...
                        <div>
                            <span class="block text-[10px] font-extrabold text-rose-700 uppercase tracking-widest">OUTSTANDING BAL.</span>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-xs font-extrabold text-slate-700 uppercase tracking-wider mb-1.5 flex items-center gap-1.5">
                            ...
                            <span>DISBURSEMENT DATE <span class="text-rose-500 font-bold">*</span></span>
                        </label>
                        <input type="date" name="payment_date" value="{{ date('Y-m-d') }}" required
                               class="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl text-xs font-bold text-slate-900 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 focus:outline-none transition-all shadow-2xs">
                    </div>

                    <div>
                        <label class="block text-xs font-extrabold text-emerald-900 uppercase tracking-wider mb-1.5 flex items-center gap-1.5">
                            <svg class="w-4 h-4 text-emerald-700" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                            <span>PAID AMOUNT (₹) <span class="text-rose-500 font-bold">*</span></span>
                        </label>
                        <input type="number" step="0.01" name="paid_amount" :max="selectedBill ? selectedBill.balance_amount : 0" required
                               class="w-full px-4 py-3 bg-emerald-50/70 border border-emerald-300/80 rounded-xl text-sm font-mono font-black text-emerald-950 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 focus:outline-none transition-all shadow-2xs"
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-xs font-extrabold text-slate-700 uppercase tracking-wider mb-1.5 flex items-center gap-1.5">
                            ...
                            <span>DISBURSE FROM BANK ACCOUNT <span class="text-rose-500 font-bold">*</span></span>
                        </label>
                        <select name="company_bank_account_id" required class="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl text-xs font-bold text-slate-900 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 focus:outline-none transition-all shadow-2xs">
                    ...
                    <div>
                        <label class="block text-xs font-extrabold text-slate-700 uppercase tracking-wider mb-1.5 flex items-center gap-1.5">
                            <svg class="w-4 h-4 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z"/></svg>
                            <span>PAYMENT MODE <span class="text-rose-500 font-bold">*</span></span>
                        </label>
                        <select name="payment_mode" required class="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl text-xs font-bold text-slate-900 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 focus:outline-none transition-all shadow-2xs">
                ...
                <div>
                    <label class="block text-xs font-extrabold text-slate-700 uppercase tracking-wider mb-1.5 flex items-center gap-1.5">
                        ...
                        <span>REFERENCE NO (CHEQUE # / UTR #)</span>
                    </label>
                    <input type="text" name="reference_no" placeholder="e.g. UTR123456789 or Chq #000123"
                           class="w-full px-4 py-3 bg-slate-50/80 border border-slate-200 rounded-xl text-xs font-bold text-slate-900 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 focus:outline-none transition-all shadow-2xs">
                </div>
                ...
                <div class="pt-4 flex items-center justify-end gap-3 border-t border-slate-100">
                    ...
                    <button type="submit"
                            class="px-6 py-3 bg-gradient-to-r from-emerald-600 to-emerald-700 hover:from-emerald-700 hover:to-emerald-800 text-white text-xs font-black uppercase tracking-wider rounded-xl transition shadow-md hover:shadow-lg flex items-center gap-2 cursor-pointer">
                        <svg class="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z"/></svg>
