...
class ChequeRealizationService
{
    ...
    public function realize(Receipt $receipt, array $data): Receipt
    {
        ...
        if (!$bankAccountId) {
            throw new \Exception(
                "Receipt #{$receipt->id} has no Company Bank Account assigned. Please select one."
            );
        }
        ...
        return DB::transaction(function () use ($receipt, $data) {
            ...
            $receipt->update([
                ...
                'realized_at'        => $data['realization_date'] ?? now(),
                'realized_by'        => $userId,
                'company_bank_account_id' => $data['company_bank_account_id'] ?? $receipt->company_bank_account_id,
            ]);

            if (isset($data['bank_reference_no']) && $data['bank_reference_no']) {
                $remarks .= ' (Bank Ref: ' . $data['bank_reference_no'] . ')';
            }
